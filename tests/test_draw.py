...
@pytest.mark.parametrize("team, matches", schedule.items())
def test_schedule(team: Team, matches: list[Match]):
    '''
    ...
    assert len(home_games) == 4, "Not enough home games"
    assert len(away_games) == 4, "Not enough away games"
    assert len(home_games.union(away_games)) == 8, "Not enough total games"
